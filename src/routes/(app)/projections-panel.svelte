...
<div class="flex min-w-0 flex-1 flex-col">
  ...
  {#if hasFinancialData}
    <div class="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-8 pb-8">
      <!-- Current projection: always present, always first -->
      ...
      {:else}
        ...
        <Card.Root class="gap-0 py-0 shadow-xs">
          <Card.Content class="flex items-center gap-4 p-4">
            ...
            <Button href={addPlanUrl}>{$_('page.dashboard.projections.addProjection')}</Button>
          </Card.Content>
        </Card.Root>
      {/if}
    </div>
  {:else}
    <!-- No balances to project from yet. Not drawn in the spec, which only
         covers a populated dashboard — kept from the previous page. -->
