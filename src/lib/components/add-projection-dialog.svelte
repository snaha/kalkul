<script lang="ts">
  ...
  interface Props {
    open: boolean
    /**
     * 'modelChanges' is the Current projection's wording ("Model changes in a
     * new projection", with an intro line); 'add' is the plain Add projection.
     */
    variant?: 'add' | 'modelChanges'
  }

  let { open = $bindable(), variant = 'add' }: Props = $props()

  // Both copies live under page.addProjection; the variant picks the subtree.
  const copy = $derived(
    variant === 'modelChanges'
      ? {
          title: $_('page.addProjection.modelChanges.title'),
          intro: $_('page.addProjection.modelChanges.intro'),
          name: $_('page.addProjection.modelChanges.name'),
          create: $_('page.addProjection.modelChanges.create'),
        }
      : {
          title: $_('page.addProjection.title'),
          intro: undefined,
          name: $_('page.addProjection.name'),
          create: $_('page.addProjection.create'),
        },
  )
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="gap-0 p-0 sm:max-w-xl">
    ...
    <div class="flex flex-col gap-4 p-4">
      {#if copy.intro}
        <Dialog.Description class="text-base text-foreground">{copy.intro}</Dialog.Description>
      {/if}
      <div class="flex flex-col gap-2">
        <Label for="{uid}-name">{copy.name}</Label>
