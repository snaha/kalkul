...
export function kalkulTools(app: App = appStore): KalkulTool[] {
  return [
    ...
    tool(
      'update_profile',
      'Merge the given fields into the profile. Array fields (investments, tangible_assets, liabilities, incomes, expenses, transfers) replace the whole list, so fetch, edit and write back the full array. Whatever is changed while working on a plan applies to that plan only: an item with plan_id belongs to that plan and stays out of financial data, so keep plan_id on such items and set it on every item added while working on a plan. Incomes and expenses carry a schedule: one_time items need transaction_year and transaction_month, recurring items need frequency, start, end and change_over_time.',
